...
const SoundFX = (() => {
  ...
  const sfx = {
    ...
    startBeachMusic() {
      ...
      // Вьетнамская пентатоника điệu Bắc — весёлый быстрый вариант
      // G4=392 A4=440 C5=523 D5=587 E5=659 G5=784 A5=880
      const N = [392, 440, 523, 587, 659, 784, 880, 1047];
      // Задорная прыгающая мелодия: скачки вверх-вниз как в народной вьетнамской музыке
      const P = [0,4,2,5, 4,2,4,0, 2,5,3,6, 4,2,1,2,
                 3,5,4,6, 2,4,2,3, 0,2,4,5, 3,2,0,1];
      let step = 0;
      const MS = 200; // 150 BPM — живо и весело

      const tick = () => {
        const freq = N[P[step % P.length]];
        // Đàn tranh (щипковая цитра) — короткий звонкий щипок
        tone(freq, 'triangle', 0.18, 0.10);
        // Glide-орнамент — вьетнамская характерная «трель» сверху вниз
        if (step % 4 === 1) tone(freq * 1.12, 'sawtooth', 0.10, 0.05, 0, freq * 0.94);
        // Sáo trúc (флейта) — весёлые акценты на каждую вторую долю
        if (step % 2 === 0) tone(freq * 2, 'sine', 0.22, 0.04);
        // Trống (барабан) — бодрый бит каждые 2 шага
        if (step % 2 === 0) noise(0.06, 200, 0.8, 0.06);
        // Акцентный удар на 1-й доле такта
        if (step % 8 === 0) tone(N[0] * 0.5, 'sine', 0.3, 0.07);
        step++;
      };
      tick();
